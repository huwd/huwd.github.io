...
class DataReport
  ...
  def books_missing_works
    @books_missing_works ||= @scraped_books.reject do |book|
      complete_wikidata_iri?(book["work_iri"])
    end
  end

  def books_edition_works
    @books_missing_works ||= @scraped_books.reject do |book|
      complete_wikidata_iri?(book["edition_iri"])
    end
  end
  ...
  def data_assembler
    ...
    @potential_work_match_count = books_missing_works.count { |book| book["wikidata_potential_work_iris"]&.any? }
